...
type RenderOverrides = Partial<OverlayState> & { onStartWithoutTracking?: () => void };
...
describe('SceneOverlays snapshots', () => {
  it('renders calibration overlay with tracking hints', () => {
    ...
    expect(markup).toMatchInlineSnapshot(`"<div class="absolute inset-0 flex items-center justify-center pointer-events-none bg-black/60 backdrop-blur-md z-50 p-4"><div class="text-center relative w-full max-w-lg"><div class="mb-8 md:mb-12 relative flex flex-col items-center justify-center scale-75 md:scale-100"><svg viewBox="0 0 200 200" class="w-64 h-64 rotate-[-90deg]"><circle cx="100" cy="100" r="85" fill="transparent" stroke="rgba(0, 255, 255, 0.1)" stroke-width="4"></circle><circle cx="100" cy="100" r="85" fill="transparent" stroke="#00ffff" stroke-width="8" stroke-dasharray="534.0707511102648" stroke-dashoffset="309.7610356439536" class="transition-all duration-100 ease-linear"></circle></svg><div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-cyan-400 font-black text-3xl">42%</div></div><h2 class="text-2xl md:text-4xl font-black italic text-cyan-400 mb-4 md:mb-8 uppercase tracking-tighter">Neural Sync Required</h2><div class="flex flex-col md:flex-row gap-2 md:gap-4 justify-center mb-4 md:mb-8"><div class="px-4 py-2 rounded border font-bold uppercase text-[10px] tracking-widest transition-colors bg-red-500/10 border-red-500/30 text-red-500">Left Hand: WAITING</div><div class="px-4 py-2 rounded border font-bold uppercase text-[10px] tracking-widest transition-colors bg-cyan-500/20 border-cyan-500 text-cyan-400">Right Hand: LOCKED</div></div><div class="bg-black/40 border border-cyan-500/30 px-4 md:px-6 py-4 rounded-xl mx-4"><p class="text-white font-mono text-xs md:text-sm tracking-widest uppercase">HOLD STEADY</p></div><div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] md:w-[40%] h-[40%] border-2 border-cyan-500/30 rounded-lg animate-pulse"></div></div></div>"`);
  });
  ...
  it('renders calibration overlay when camera access is blocked', () => {
    ...
    expect(markup).toMatchInlineSnapshot(`"<div class=\"absolute inset-0 flex items-center justify-center pointer-events-none bg-black/60 backdrop-blur-md z-50 p-4\"><div class=\"text-center relative w-full max-w-lg\"><div class=\"mb-8 md:mb-12 relative flex flex-col items-center justify-center scale-75 md:scale-100\"><svg viewBox=\"0 0 200 200\" class=\"w-64 h-64 rotate-[-90deg]\"><circle cx=\"100\" cy=\"100\" r=\"85\" fill=\"transparent\" stroke=\"rgba(0, 255, 255, 0.1)\" stroke-width=\"4\"></circle><circle cx=\"100\" cy=\"100\" r=\"85\" fill=\"transparent\" stroke=\"#00ffff\" stroke-width=\"8\" stroke-dasharray=\"534.0707511102648\" stroke-dashoffset=\"309.7610356439536\" class=\"transition-all duration-100 ease-linear\"></circle></svg><div class=\"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-cyan-400 font-black text-3xl\">42%</div></div><h2 class=\"text-2xl md:text-4xl font-black italic text-cyan-400 mb-4 md:mb-8 uppercase tracking-tighter\">Neural Sync Required</h2><div class=\"flex flex-col md:flex-row gap-2 md:gap-4 justify-center mb-4 md:mb-8\"><div class=\"px-4 py-2 rounded border font-bold uppercase text-[10px] tracking-widest transition-colors bg-red-500/10 border-red-500/30 text-red-500\">Left Hand: WAITING</div><div class=\"px-4 py-2 rounded border font-bold uppercase text-[10px] tracking-widest transition-colors bg-cyan-500/20 border-cyan-500 text-cyan-400\">Right Hand: LOCKED</div></div><div class=\"bg-black/40 border border-cyan-500/30 px-4 md:px-6 py-4 rounded-xl mx-4\"><p class=\"text-white font-mono text-xs md:text-sm tracking-widest uppercase\">Camera offline. Reconnect or select a webcam, then press Retry to restore the feed.</p></div><div class=\"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] md:w-[40%] h-[40%] border-2 border-cyan-500/30 rounded-lg animate-pulse\"></div></div></div>\"`);
  });
  ...
  it('renders calibration overlay fallback CTA when provided', () => {
    ...
    expect(markup).toMatchInlineSnapshot(`"<div class=\"absolute inset-0 flex items-center justify-center pointer-events-auto bg-black/60 backdrop-blur-md z-50 p-4\"><div class=\"text-center relative w-full max-w-lg\"><div class=\"mb-8 md:mb-12 relative flex flex-col items-center justify-center scale-75 md:scale-100\"><svg viewBox=\"0 0 200 200\" class=\"w-64 h-64 rotate-[-90deg]\"><circle cx=\"100\" cy=\"100\" r=\"85\" fill=\"transparent\" stroke=\"rgba(0, 255, 255, 0.1)\" stroke-width=\"4\"></circle><circle cx=\"100\" cy=\"100\" r=\"85\" fill=\"transparent\" stroke=\"#00ffff\" stroke-width=\"8\" stroke-dasharray=\"534.0707511102648\" stroke-dashoffset=\"309.7610356439536\" class=\"transition-all duration-100 ease-linear\"></circle></svg><div class=\"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-cyan-400 font-black text-3xl\">42%</div></div><h2 class=\"text-2xl md:text-4xl font-black italic text-cyan-400 mb-4 md:mb-8 uppercase tracking-tighter\">Neural Sync Required</h2><div class=\"flex flex-col md:flex-row gap-2 md:gap-4 justify-center mb-4 md:mb-8\"><div class=\"px-4 py-2 rounded border font-bold uppercase text-[10px] tracking-widest transition-colors bg-red-500/10 border-red-500/30 text-red-500\">Left Hand: WAITING</div><div class=\"px-4 py-2 rounded border font-bold uppercase text-[10px] tracking-widest transition-colors bg-cyan-500/20 border-cyan-500 text-cyan-400\">Right Hand: LOCKED</div></div><div class=\"bg-black/40 border border-cyan-500/30 px-4 md:px-6 py-4 rounded-xl mx-4\"><p class=\"text-white font-mono text-xs md:text-sm tracking-widest uppercase\">We cannot see your hands. Move them into view or start without tracking.</p></div><div class=\"mt-6 flex flex-col gap-3 items-center pointer-events-auto\"><button class=\"px-4 py-2 bg-cyan-600/80 hover:bg-cyan-500 text-white font-bold uppercase tracking-widest rounded shadow-lg transition\">Start without hand tracking</button><p class=\"text-[11px] text-white/60 uppercase tracking-[0.2em]\">Mouse + keyboard controls will be enabled.</p></div><div class=\"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] md:w-[40%] h-[40%] border-2 border-cyan-500/30 rounded-lg animate-pulse\"></div></div></div>\"`);
  });

  it('shows actionable messaging when no device is detected', () => {
    const message = 'No camera detected. Plug in a webcam, ensure it is enabled, then press Retry to reconnect.';
    const markup = renderPhase('CALIBRATING', {
  ...
  it('shows actionable messaging when permission is denied', () => {
    const message = 'Camera permission denied. Allow access in your browser settings and hit Retry.';
    const markup = renderPhase('CALIBRATING', {
      calibrationStatus: { stalled: false, cameraReady: false, message },
    });
